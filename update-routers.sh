...
for router in "${routers[@]}"; do
    ...
    # Generate ready-to-use BIRD configuration for each router
    ...
    if [ "$router" == "dcg-1.router.nl.coloclue.net" ] || [ "$router" == "dcg-2.router.nl.coloclue.net" ]; then
        ...
        # EUNetworks-specific configuration
...
if [ "$1" == "push" ]; then
    ...
    # upload configuration and reload BIRD
    for router in "${routers[@]}"; do
    ...
    # update RIPE Database if new peers are added
    php /opt/coloclue/update-as8283-ripe.php

	# kill ssh-agent (TODO: why do we need to kill ssh-agent twice?)
	eval "$(ssh-agent -k)"
